...
class Network_Classifier(object):
	...
	def __init__(self, network_layers, configs, num_labels): 	
		...
		if 'update_function' not in configs:	
			configs['update_function'] = '_conventional_updates'
		...
		for layer in network_layers:
			...
			if 'LSTM' in layer[0] :
				...
				elif layer[0]=='ModifiedLSTM':
					layerx = ModifiedLSTMLayer(rng = self.rng, dim_in = dim_in, dim_out = dim_out, window = window, pooling = pooling, Wx=[None,None,None,None], Wh=[None,None,None,None], b=[None,None,None,None], Wfinal=None, bfinal=None, use_bias = use_bias, use_last_output = use_last_output)
			
			elif 'Convolution' in layer[0] :
				dim_in = int(configs['dim_in'])
				if configs['dim_out'] == 'labels':
					configs['dim_out'] = num_labels
				dim_out = int(configs['dim_out'])
				...
				elif layer[0]=='ModifiedConvolution':
					layerx = ModifiedConvolutionLayer(rng = self.rng, dim_in = dim_in, dim_out = dim_out, window = window, activation = activation, pooling = pooling, W=None, b=None, Wfinal=None, bfinal=None, use_bias = use_bias)
...
def load_configs(config_file):
	...
	configs['update_function'] = 'conventional_updates' if 'update_function' not in configs else configs['update_function'].lower()
